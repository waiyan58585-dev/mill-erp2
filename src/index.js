...
export default function MillERP() {
  ...
  // Opening Stock Form (လက်ကျန်စာရင်းသွင်းရန်)
  ...
  const handleDeleteJob = (jobId) => {
    if (userRole !== 'admin') {
        showMessage('သင့်တွင် ဤမှတ်တမ်းအား ဖျက်သိမ်းခွင့် မရှိပါ။ (Admin သာ ဖျက်ခွင့်ရှိသည်)');
        return;
    }
  ...
  const renderDashboardView = () => {
    ...
    jobs.forEach(job => {
        if (job.deliveryLogs) {
            todayDeliveries += job.deliveryLogs.filter(log => log.date === todayStr).length;
        }
    });
                        ...
                        <div className="space-y-3">
                            {jobs.filter(j => j.status === 'ready_to_bill' || j.status === 'ready_to_bill_advance').slice(0, 5).map(job => (
                                <div key={job.id} className="flex justify-between items-center p-3 border border-slate-100 rounded-xl hover:bg-slate-50">
                                    <div>
                                        <p className="font-bold text-slate-800">{job.customer}</p>
                                        <p className="text-xs text-slate-500">{job.id} • {job.paddyType}</p>
                                    </div>
        ...
        return (
            <div className="animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-blue-500">
                        <p className="text-sm font-bold text-slate-500 mb-2">ယနေ့ စပါးအဝင်</p>
                        <p className="text-3xl font-black text-slate-800">{todayPaddyBags} <span className="text-sm">တင်း</span></p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-orange-500">
                        <p className="text-sm font-bold text-slate-500 mb-2">အခြောက်ခံရန် ကျန် (အစို)</p>
                        <p className="text-3xl font-black text-orange-600">{pendingDry} <span className="text-sm">စာရင်း</span></p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-orange-500">
                        <p className="text-sm font-bold text-slate-500 mb-2">စက်လည်နေဆဲ</p>
                        <p className="text-3xl font-black text-orange-600">{dryingNow} <span className="text-sm">စာရင်း</span></p>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4">လုပ်ဆောင်ရန်</h3>
            ...
            <div className="animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-purple-500">
                        <p className="text-sm font-bold text-slate-500 mb-2">ကြိတ်ခွဲရန် စောင့်ဆိုင်းဆဲ</p>
                        <p className="text-3xl font-black text-purple-600">{pendingMill} <span className="text-sm">စာရင်း</span></p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-blue-500">
                        <p className="text-sm font-bold text-slate-500 mb-2">Color Sort ရန် ကျန်</p>
                        <p className="text-3xl font-black text-blue-600">{pendingSort} <span className="text-sm">စာရင်း</span></p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-green-500">
                        <p className="text-sm font-bold text-slate-500 mb-2">ယနေ့ ဆန်ထုတ်ပေးမှု</p>
                        <p className="text-3xl font-black text-green-600">{todayDeliveries} <span className="text-sm">ကြိမ်</span></p>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4">လုပ်ဆောင်ရန်</h3>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-8">
          ...
          <form onSubmit={handleAddJob} className="space-y-4">
            <div className="flex bg-slate-100 p-1 rounded-xl w-full max-w-md mb-4 border border-slate-200">
              ...
              <button type="button" onClick={() => setNewJob({...newJob, entryType: 'nawali'})} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${newJob.entryType === 'nawali' ? 'bg-white text-orange-700 shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>နဝလီ (Color Sort အဝင်)</button>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 max-w-3xl">
                ...
                <form onSubmit={handleOsSubmit} className="space-y-5">
                    <div className="flex bg-slate-100 p-1 rounded-xl w-full max-w-md border border-slate-200">
                        <button type="button" onClick={() => setOsInput({...osInput, owner: 'merchant'})} className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${osInput.owner === 'merchant' ? 'bg-white text-blue-700 shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>ကုန်သည်ပိုင် (Merchant)</button>
                        <button type="button" onClick={() => setOsInput({...osInput, owner: 'mill'})} className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${osInput.owner === 'mill' ? 'bg-white text-cyan-700 shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>စက်ပိုင် (Mill Owned)</button>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div><label className="block text-xs font-bold text-slate-600 mb-1.5">ရက်စွဲ</label><input type="date" value={osInput.date} onChange={e=>setOsInput({...osInput, date: e.target.value})} className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 text-sm outline-none focus:border-blue-500" required/></div>
                        {osInput.owner === 'merchant' && (
                            <div><label className="block text-xs font-bold text-slate-600 mb-1.5">ဖောက်သည်အမည်</label><input type="text" value={osInput.customer} onChange={e=>setOsInput({...osInput, customer: e.target.value})} className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 text-sm outline-none focus:border-blue-500" required placeholder="အမည်"/></div>
                        )}
                        <div>
                            <label className="block text-xs font-bold text-slate-600 mb-1.5">ပစ္စည်းအမျိုးအစား</label>
                            <select value={osInput.category} onChange={e=>setOsInput({...osInput, category: e.target.value})} className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 text-sm font-bold outline-none focus:border-blue-500">
                        ...
                        <div><label className="block text-xs font-bold text-slate-600 mb-1.5">အရေအတွက် ({osInput.category === 'paddy' ? 'တင်း' : 'အိတ်'})</label><input type="number" min="1" value={osInput.qty} onChange={e=>setOsInput({...osInput, qty: e.target.value})} className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 text-sm font-bold outline-none focus:border-blue-500" required placeholder="0"/></div>
                        <div className="col-span-1 md:col-span-2"><label className="block text-xs font-bold text-slate-600 mb-1.5">ဂိုဒေါင်/ထားသိုရာနေရာ</label><input type="text" list="osStorageLocations" value={osInput.storage} onChange={e=>setOsInput({...osInput, storage: e.target.value})} className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 text-slate-900 text-sm outline-none focus:border-blue-500" required placeholder="နေရာ..."/></div>
                    ...
                    <div className="pt-4 mt-2">
                        <button disabled={isLoading} type="submit" className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold shadow-lg hover:bg-slate-900 transition-colors">အတည်ပြု စာရင်းသွင်းမည်</button>
                    </div>
                </form>
            </div>
        </div>
  ...
  const renderPaddyWarehouseView = () => {
    ...
    return (
      <div className="animate-in fade-in duration-300 space-y-8">
        <h2 className="text-2xl font-bold text-slate-800 flex items-center"><Package className="mr-3 text-orange-600"/> စပါး / နဝလီ ဂိုဒေါင် နှင့် အခြောက်ခံစက်</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white rounded-2xl border border-orange-200 shadow-sm flex flex-col">
                ...
                {waitingDryJobs.map(job => (
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <span className="text-sm font-semibold text-slate-500 mt-1 block">{job.id} • {job.paddyType}</span>
                        </div>
                        <div className="text-right">
                            ...
                            <span className="text-xs text-slate-500"><MapPin size={12} className="inline mr-1"/>{job.storage} (ဂိုဒေါင်)</span>
                        </div>
                    </div>
                ...
                {waitingDryJobs.length === 0 && <p className="text-slate-400 text-sm font-bold text-center py-6">စောင့်ဆိုင်းစာရင်း မရှိပါ။</p>}
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-red-200 shadow-sm flex flex-col">
                ...
                {dryingJobs.map(job => (
                    ...
                    {activeJobId === job.id + '-finish' ? (
                        ...
                        <div><label className="text-xs font-bold text-red-800 block mb-1.5">ကျန်ရှိမည့် တင်း (အခြောက်)</label><input type="number" value={dryQtyInput} onChange={e=>setDryQtyInput(e.target.value)} className="w-full p-3 border-2 border-red-300 rounded-xl outline-none bg-white text-slate-900 font-black" required/></div>
                        <div><label className="text-xs font-bold text-slate-600 block mb-1.5">သိုလှောင်မည့် စပါးဂိုဒေါင် အသစ်</label><input type="text" list="storageLocations" value={dryStorageInput} onChange={e=>setDryStorageInput(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-lg text-sm bg-white text-slate-900" required/></div>
                        
                        <div className="flex gap-2 pt-2">
                            <button disabled={isLoading} onClick={() => setActiveJobId(null)} className="flex-1 bg-slate-100 text-slate-700 py-3 rounded-xl text-sm font-bold">ပယ်ဖျက်</button>
                        </div>
                    ...
                    ) : <div className="pl-3 mt-4 border-t border-slate-100 pt-4"><button onClick={() => setActiveJobId(job.id + '-finish')} className="w-full bg-red-50 text-red-700 py-3 rounded-xl text-sm font-bold hover:bg-red-100 shadow-sm">အခြောက်ခံပြီးစီး</button></div>}
                    </div>
                ))}
                {dryingJobs.length === 0 && <p className="text-slate-400 text-sm font-bold text-center py-6">အခြောက်ခံနေဆဲ မရှိပါ။</p>}
                </div>
            </div>
        </div>
      </div>
                ...
                <div className="bg-purple-50/50 p-6 rounded-2xl border border-purple-100 mt-6">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-blue-100">
                      ...
                      <input type="text" list="riceStorageList" value={millInput.broken12Storage} onChange={e=>setMillInput({...millInput, broken12Storage: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-xs bg-slate-50 text-slate-900" placeholder="ထားသိုရာနေရာ"/>
                    </div>
                    {/* Broken 234 Input */}
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-blue-100">
                      ...
                      <input type="text" list="riceStorageList" value={millInput.broken234Storage} onChange={e=>setMillInput({...millInput, broken234Storage: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-xs bg-slate-50 text-slate-900" placeholder="ထားသိုရာနေရာ"/>
                    </div>
                    {/* Bran Input */}
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-orange-100">
                      ...
                      <input type="text" list="riceStorageList" value={millInput.branStorage} onChange={e=>setMillInput({...millInput, branStorage: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-xs bg-slate-50 text-slate-900" placeholder="ထားသိုရာနေရာ"/>
                    </div>
                  </div>
  ...
  const renderSortingView = () => {
    ...
    return (
      <div className="animate-in fade-in duration-300">
        <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center"><ScanLine className="mr-3 text-blue-600"/> Color Sorting ဌာန</h2>
        <div className="space-y-5">
          {jobs.filter(j => j.status === 'waiting_sort').map(job => {
            ...
            return (
              <div key={job.id} className="bg-white p-6 rounded-2xl border-l-4 border-l-blue-500 shadow-sm">
                ...
                ) : <div className="border-t border-slate-100 pt-5 mt-2"><button onClick={() => setActiveJobId(job.id)} className="w-full bg-slate-50 hover:bg-blue-50 text-blue-700 border border-blue-200 py-3.5 rounded-xl font-bold text-sm">Sorting ရလဒ် စာရင်းသွင်းမည်</button></div>}
              </div>
            );
          })}
  ...
  const renderRiceWarehouseView = () => {
    // Show jobs that have outputs (waiting_bill, billed, or partially delivered)
    ...
    return (
      <div className="animate-in fade-in duration-300 relative">
        <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center"><Factory className="mr-3 text-green-600"/> ဆန် နှင့် ထွက်ကုန် ဂိုဒေါင် (Delivery)</h2>
              ...
              <table className="w-full text-left min-w-[900px]">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-800">
                  {riceJobs.map(job => {
                    ...
                    return (
                      <tr key={job.id} className={`hover:bg-slate-50 ${isFullyDelivered ? 'opacity-50' : ''}`}>
                        ...
                        <td className="p-4 align-top">
                           {isFullyDelivered ? (
                               ...
                               <div className="flex flex-wrap gap-2">
                                  ...
                                  {rem.out2 > 0 && job.billData?.byproductOption !== 'sell' && <div className="text-xs font-bold bg-slate-100 text-slate-700 px-2 py-1 rounded border border-slate-200">{labels[1]}: {rem.out2} အိတ်</div>}
                           ...
                           {logs.length > 0 && (
                               <div className="mt-3 pt-3 border-t border-slate-100 border-dashed space-y-1">
                                   <p className="text-[10px] font-bold uppercase text-slate-400">ထုတ်ယူမှု မှတ်တမ်းများ:</p>
                                   {logs.map((log, idx) => (
                                       <div key={idx} className="text-xs text-slate-600 bg-slate-50 p-1.5 rounded flex gap-3">
                                           <span>{log.date}</span>
                                           <span>ကား: {log.carNo}</span>
                                           <span className="text-slate-400">({Object.values(log.items).reduce((a,b)=>a+(Number(b)||0),0)} အိတ်)</span>
                                       </div>
                                   ))}
                               </div>
                           )}
                        ...
                        <td className="p-4 align-top text-center">
                            {!isFullyDelivered && (
                                <button onClick={() => handleDeliveryClick(job)} className="bg-slate-800 hover:bg-slate-900 text-white font-bold px-4 py-2.5 rounded-xl text-xs shadow-md transition-colors flex items-center mx-auto">
                  ...
                  {riceJobs.length === 0 && <tr><td colSpan="3" className="p-6 text-center text-slate-400 font-bold">ဆန်ဂိုဒေါင်တွင် လာထုတ်ရန်ကျန်သူ မရှိပါ။</td></tr>}
                </tbody>
              </table>
           </div>
        </div>
                ...
                <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col relative z-10">
                    ...
                    <form onSubmit={handleDeliverySubmit} className="p-6 overflow-y-auto flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="block text-xs font-bold text-slate-700 mb-1.5">ထုတ်ယူသည့် ရက်စွဲ</label><input type="date" required value={deliveryInput.date} onChange={e=>setDeliveryInput({...deliveryInput, date: e.target.value})} className="w-full p-2.5 border border-slate-300 rounded-lg outline-none bg-slate-50 text-slate-900 text-sm"/></div>
                            <div><label className="block text-xs font-bold text-slate-700 mb-1.5">ကားနံပါတ်</label><input type="text" required placeholder="ဥပမာ - YGN 1A-1234" value={deliveryInput.carNo} onChange={e=>setDeliveryInput({...deliveryInput, carNo: e.target.value})} className="w-full p-2.5 border border-slate-300 rounded-lg outline-none focus:border-blue-500 bg-slate-50 text-slate-900 text-sm"/></div>
                            <div className="col-span-2"><label className="block text-xs font-bold text-slate-700 mb-1.5">လာရောက်ထုတ်ယူသူ အမည် / ဖုန်း</label><input type="text" required placeholder="အမည် (သို့) ဖုန်းနံပါတ်" value={deliveryInput.driverName} onChange={e=>setDeliveryInput({...deliveryInput, driverName: e.target.value})} className="w-full p-2.5 border border-slate-300 rounded-lg outline-none focus:border-blue-500 bg-slate-50 text-slate-900 text-sm"/></div>
                        </div>

                        <div className="border-t border-slate-200 pt-4 mt-4 space-y-3">
                            <p className="text-xs font-bold uppercase text-slate-500 mb-2">ထုတ်ပေးမည့် ပစ္စည်းများ (အိတ်)</p>
                            
                            {deliveryModal.available.out1 > 0 && (
                                <div className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-200">
                                    <span className="text-sm font-bold text-slate-700">{getSortingLabels(deliveryModal.job.paddyType)[0]} (Max: {deliveryModal.available.out1})</span>
                                    <input type="number" min="0" max={deliveryModal.available.out1} placeholder="0" value={deliveryInput.items.out1 || ''} onChange={e=>setDeliveryInput({...deliveryInput, items: {...deliveryInput.items, out1: e.target.value}})} className="w-24 p-2 border border-slate-300 rounded text-right outline-none focus:border-blue-500 font-bold"/>
                                </div>
                            )}
                            {deliveryModal.available.out2 > 0 && deliveryModal.job.billData?.byproductOption !== 'sell' && (
                                <div className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-200">
                                    <span className="text-sm font-bold text-slate-700">{getSortingLabels(deliveryModal.job.paddyType)[1]} (Max: {deliveryModal.available.out2})</span>
                                    <input type="number" min="0" max={deliveryModal.available.out2} placeholder="0" value={deliveryInput.items.out2 || ''} onChange={e=>setDeliveryInput({...deliveryInput, items: {...deliveryInput.items, out2: e.target.value}})} className="w-24 p-2 border border-slate-300 rounded text-right outline-none focus:border-blue-500 font-bold"/>
                                </div>
                            )}
                            {deliveryModal.available.out3 > 0 && deliveryModal.job.billData?.rejectOption !== 'sell' && (
                                <div className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-200">
                                    <span className="text-sm font-bold text-slate-700">{getSortingLabels(deliveryModal.job.paddyType)[2]} (Max: {deliveryModal.available.out3})</span>
                                    <input type="number" min="0" max={deliveryModal.available.out3} placeholder="0" value={deliveryInput.items.out3 || ''} onChange={e=>setDeliveryInput({...deliveryInput, items: {...deliveryInput.items, out3: e.target.value}})} className="w-full p-2 border border-slate-300 rounded text-right outline-none focus:border-blue-500 font-bold"/>
                                </div>
                            )}
                            {deliveryModal.available.broken12 > 0 && (
                                <div className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-200">
                                    <span className="text-sm font-bold text-slate-700">၁၂ ဆန်ကွဲ (Max: {deliveryModal.available.broken12})</span>
                                    <input type="number" min="0" max={deliveryModal.available.broken12} placeholder="0" value={deliveryInput.items.broken12 || ''} onChange={e=>setDeliveryInput({...deliveryInput, items: {...deliveryInput.items, broken12: e.target.value}})} className="w-24 p-2 border border-slate-300 rounded text-right outline-none focus:border-blue-500 font-bold"/>
                                </div>
                            )}
                            {deliveryModal.available.broken234 > 0 && (
                                <div className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-200">
                                    <span className="text-sm font-bold text-slate-700">၂၃၄ ဆန်ကွဲ (Max: {deliveryModal.available.broken234})</span>
                                    <input type="number" min="0" max={deliveryModal.available.broken234} placeholder="0" value={deliveryInput.items.broken234 || ''} onChange={e=>setDeliveryInput({...deliveryInput, items: {...deliveryInput.items, broken234: e.target.value}})} className="w-24 p-2 border border-slate-300 rounded text-right outline-none focus:border-blue-500 font-bold"/>
                                </div>
                            )}
                            {deliveryModal.available.bran > 0 && deliveryModal.job.billData?.branOption !== 'sell' && (
                                <div className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-200">
                                    <span className="text-sm font-bold text-slate-700">ဖွဲနု (Max: {deliveryModal.available.bran})</span>
                                    <input type="number" min="0" max={deliveryModal.available.bran} placeholder="0" value={deliveryInput.items.bran || ''} onChange={e=>setDeliveryInput({...deliveryInput, items: {...deliveryInput.items, bran: e.target.value}})} className="w-24 p-2 border border-slate-300 rounded text-right outline-none focus:border-blue-500 font-bold"/>
                                </div>
                            )}
                        </div>
                        
                        <button disabled={isLoading} type="submit" className="w-full mt-4 py-3.5 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-bold text-lg shadow-lg">
      ...
      <div className="animate-in fade-in duration-300">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 mb-8 shadow-sm">
          ...
          <div className="relative max-w-xl">
            <input type="text" placeholder="ဖောက်သည် အမည် (သို့) ဘောက်ချာ ID ရိုက်ထည့်ပါ..." value={adminSearchQuery} onChange={e => setAdminSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3.5 border-2 border-slate-200 rounded-xl outline-none focus:border-blue-500 font-bold bg-slate-50 text-slate-900"/>
          ...
          {searchedJobs.map(job => {
            ...
            return (
              <div key={job.id} className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden flex flex-col">
                <div className={`${isAdvance ? 'bg-green-700' : 'bg-slate-900'} p-6 text-white flex justify-between items-center relative`}>
                      ...
                      {isAdvance && (
                        <>
                           <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                             ...
                             <input type="number" value={billInput.dryingRate} onChange={e=>setBillInput({...billInput, dryingRate: e.target.value})} className="w-full p-3 border-2 border-slate-200 bg-slate-50 text-slate-900 rounded-xl outline-none focus:border-green-500 font-bold" placeholder="ကျပ်"/>
                           ...
                           <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                             ...
                             <input type="number" value={billInput.otherExp} onChange={e=>setBillInput({...billInput, otherExp: e.target.value})} className="w-full p-3 border-2 border-slate-200 bg-slate-50 text-slate-900 rounded-xl outline-none focus:border-green-500 font-bold" placeholder="0"/>
                           </div>
                        </>
                      )}
                      ...
                      {!isAdvance && (
                         <>
                            {job.wasWet && !hasPaidAdvance && (
                              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                <label className="block text-sm font-bold text-slate-800 mb-3">အခြောက်ခံခ နှုန်းထား (၁ တင်းလျှင်)</label>
                                <input type="number" value={billInput.dryingRate} onChange={e=>setBillInput({...billInput, dryingRate: e.target.value})} className="w-full p-3 border-2 border-slate-200 bg-slate-50 text-slate-900 rounded-xl font-bold" placeholder="ကျပ်"/>
                              </div>
                            )}
                            {!isDryOnly && (
                              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                ...
                                <input type="number" value={isNawali ? billInput.sortingRate : billInput.millingRate} onChange={e=> isNawali ? setBillInput({...billInput, sortingRate: e.target.value}) : setBillInput({...billInput, millingRate: e.target.value})} className="w-full p-3 border-2 border-slate-200 bg-slate-50 text-slate-900 rounded-xl font-bold" placeholder="ကျပ်"/>
                              </div>
                            )}
                            {!isDryOnly && (branQty > 0 || byproductQty > 0 || rejectQty > 0) && (
                              <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-200 shadow-sm space-y-4 text-slate-900">
                                <h4 className="text-sm font-black text-blue-800 uppercase tracking-wider mb-2">စက်သို့ပြန်ရောင်း၍ ခုနှိမ်ခြင်း</h4>
                                {branQty > 0 && <div className="p-3 bg-white rounded-xl border border-blue-100"><label className="block text-xs font-bold mb-2">ဖွဲနု ({branQty} အိတ်) ရောင်းမည်လား?</label><select value={billInput.branOption} onChange={e=>setBillInput({...billInput, branOption: e.target.value})} className="w-full p-2 border mb-2 bg-slate-50 text-slate-900 outline-none rounded-lg"><option value="take">ဖောက်သည် ယူမည်</option><option value="sell">စက်သို့ ရောင်းမည်</option></select>{billInput.branOption === 'sell' && <input type="number" placeholder="၁ အိတ် နှုန်း" value={billInput.branRate} onChange={e=>setBillInput({...billInput, branRate: e.target.value})} className="w-full p-2 border rounded bg-slate-50 text-slate-900 outline-none"/>}</div>}
                                {byproductQty > 0 && <div className="p-3 bg-white rounded-xl border border-blue-100"><label className="block text-xs font-bold mb-2">Byproduct ({byproductQty} အိတ်) ရောင်းမည်လား?</label><select value={billInput.byproductOption} onChange={e=>setBillInput({...billInput, byproductOption: e.target.value})} className="w-full p-2 border mb-2 bg-slate-50 text-slate-900 outline-none rounded-lg"><option value="take">ဖောက်သည် ယူမည်</option><option value="sell">စက်သို့ ရောင်းမည်</option></select>{billInput.byproductOption === 'sell' && <input type="number" placeholder="၁ အိတ် နှုန်း" value={billInput.byproductRate} onChange={e=>setBillInput({...billInput, byproductRate: e.target.value})} className="w-full p-2 border rounded bg-slate-50 text-slate-900 outline-none"/>}</div>}
                                {rejectQty > 0 && <div className="p-3 bg-white rounded-xl border border-blue-100"><label className="block text-xs font-bold mb-2">Reject ({rejectQty} အိတ်) ရောင်းမည်လား?</label><select value={billInput.rejectOption} onChange={e=>setBillInput({...billInput, rejectOption: e.target.value})} className="w-full p-2 border mb-2 bg-slate-50 text-slate-900 outline-none rounded-lg"><option value="take">ဖောက်သည် ယူမည်</option><option value="sell">စက်သို့ ရောင်းမည်</option></select>{billInput.rejectOption === 'sell' && <input type="number" placeholder="၁ အိတ် နှုန်း" value={billInput.rejectRate} onChange={e=>setBillInput({...billInput, rejectRate: e.target.value})} className="w-full p-2 border rounded bg-slate-50 text-slate-900 outline-none"/>}</div>}
                              </div>
                            )}
                            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                               <label className="block text-sm font-bold text-slate-800 mb-2">အခြား ကုန်ကျစရိတ်</label>
                               <input type="number" value={billInput.otherExp} onChange={e=>setBillInput({...billInput, otherExp: e.target.value})} className="w-full p-3 border-2 border-slate-200 bg-slate-50 text-slate-900 rounded-xl font-bold" placeholder="0"/>
                            </div>
                         </>
                      )}
                    </div>
                    ...
                    <div className="mt-auto bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                      <div className="border-b border-slate-200 pb-4 mb-4">
                         <div className="flex justify-between items-end">
                           ...
                           <span className={`text-3xl font-black ${netTotal < 0 ? 'text-green-600' : 'text-slate-800'}`}>{Math.abs(netTotal).toLocaleString()} Ks</span>
                         </div>
                      </div>
                      {Math.abs(netTotal) > 0 && (
        ...
        <div className="space-y-4 pb-20">
          {filteredCustomers.map(cust => (
            <div key={cust.name} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden transition-all">
              <div className="bg-slate-50 hover:bg-blue-50/50 p-5 flex justify-between items-center transition-colors">
                <div className="flex items-center gap-4 cursor-pointer flex-1" onClick={() => setExpandedCustomer(expandedCustomer === cust.name ? null : cust.name)}>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 font-medium mt-0.5">လုပ်ငန်းစဉ်/ငွေပေးချေမှု: <span className="text-blue-600 font-bold">{cust.history.length}</span> ကြိမ်</p>
                  </div>
                </div>
  ...
  const renderMenu = () => {
    ...
    if (userRole === 'admin') {
      menus = [
        { id: 'dashboard', name: 'Dashboard', icon: LayoutDashboard, color: 'text-blue-600', activeBg: 'bg-blue-50 border-blue-100 text-slate-800' },
        { id: 'gate', name: 'စပါးအဝင်/ဂိတ်', icon: Home, color: 'text-blue-600', activeBg: 'bg-blue-50 border-blue-100 text-slate-800' },
        { id: 'paddy_warehouse', name: 'စပါး/နဝလီ ဂိုဒေါင်', icon: Factory, color: 'text-orange-600', activeBg: 'bg-orange-50 border-orange-100 text-slate-800' },
  ...
  return (
    <div className="flex h-screen bg-[#f1f5f9] font-sans relative overflow-hidden">
      ...
      {!isConfigured && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-100">
          <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg text-center border border-slate-200">
            ...
            <p className="text-slate-600 font-medium text-sm">src/index.js ဖိုင်၏ အပေါ်ဆုံးတွင် သင့်၏ URL နှင့် Key အစစ်ကို အစားထိုးထည့်သွင်းပေးပါ။</p>
          </div>
        </div>
      )}
      ...
      {userRole && isConfigured && (
          <>
            <div className="w-[280px] bg-white border-r border-slate-200 flex flex-col shadow-sm z-20 shrink-0">
